...
export default async function handler(req, res) {
  ...
  try {
    ...
    // query a ser executada com a clausula ORDER BY RAND() que indica um sorteio de pokemon
    const sql = `SELECT * FROM Pokemon ORDER BY RAND() LIMIT 1;`; // query SQL atribuida a variavel
    ...
    if (results) {
      ...
      res.status(200).json({
        message: "Esta é uma rota de API sort",
        status: true,
        data: results,
      });
    } else {
